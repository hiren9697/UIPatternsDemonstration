//
//  WTBasicMVVMViewController.swift
//  ECommerceScreens
//
...
public class WTBasicMVVMViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    private let viewModel: WTBasicMVVMViewModel
    private let shouldAnimate: Bool
    private let onFinish: FinishCompletion
    
    init(items: [WalkthroughItem],
                viewModel: WTBasicMVVMViewModel,
                shouldAnimate: Bool = true,
    ...
    public override func viewDidLoad() {
        ...
        setupPageControl()
        bind()
        viewModel.itemScrolled(currentItem: 0)
    }
    ...
    private func bind() {
        viewModel.onItemScroll = {[weak self] currentItem in
        ...
        viewModel.onPreviousButtonVisibilityChange = {[weak self] isVisible in
            self?.previousButton.isHidden = !isVisible
        }
        viewModel.onCurrentItemTrackingTextChange = {[weak self] text in
            self?.currentItemTrackingLabel.text = text
        }
    }
    ...
    private func updateCurrentItem() {
        ...
        viewModel.itemScrolled(currentItem: currentItemIndex)
    }
