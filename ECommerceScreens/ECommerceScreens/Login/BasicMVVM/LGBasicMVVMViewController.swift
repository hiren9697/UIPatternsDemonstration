//
//  LGBasicMVVMViewController.swift
//  ECommerceScreens
//
...
public class LGBasicMVVMViewController: UIViewController {
    ...
    let viewModel: LGBasicMVVMViewModel
    ...
    @objc func loginTap() {
        viewModel.login(rawEmail: emailField.text, rawPassword: passwordField.text)
    }
    
    // MARK: - Binding
    private func bind() {
        viewModel.onShowToast = { [weak self] message in
            self?.toast.present(message: message)
        }
        viewModel.onLoginProgressStateChange = {[weak self] isProgressVisible in
            if isProgressVisible {
                self?.loginButton.showProgress()
            } else {
